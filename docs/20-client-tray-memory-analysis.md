...
2026-07-02 已继续完成低风险内存治理：

- `LogService.LoadLogsAsync()` 改为逐行读取执行日志，不再使用 `ReadAllLinesAsync()` 一次性载入完整文件。
- 执行历史 UI 默认最多加载最近 `500` 条记录，避免长期高频规则执行后打开历史页面时内存突增。
- `RuleSchedulerService.InitializeLastRanFromLogs()` 改为逐行读取最近两天执行日志，不再用 `ReadAllLines()` 一次性载入。
- 调度器启动恢复用的内存执行历史限制为最近 `1000` 条，保留自动恢复判断所需上下文，同时避免历史无限增长。
- 诊断包 `runtime-summary.json` 和 `diagnostic-summary.json` 新增脱敏进程指标：
  - 工作集。
  - 私有内存。
  - 托管堆估算。
  - 句柄数。
...
上述诊断指标不记录命令行参数、用户路径、完整设备指纹或服务端拦截细节。

## 5. 已实施的第三阶段优化

2026-07-03 已继续处理“打开过主窗口后再隐藏到托盘仍占用较高”的场景：

- 当主窗口由 `BackgroundRuntime` 托管时，关闭动作若选择“隐藏到托盘”，不再只是 `Hide()`。
- 托盘隐藏会关闭并释放 `MainWindow` 与 `MainViewModel`，后台仅保留 `BackgroundRuntime`、托盘图标、规则调度、更新服务和必要日志服务。
- 下次从托盘点击“打开主窗口”时重新创建 `MainWindow`。
- `MainViewModel.Shutdown()` 增加调度器事件与策略事件退订，避免后台服务继续引用已关闭 UI。
- 关闭释放时清理 UI 集合与 `DataContext`，降低已打开 UI 的常驻内存压力。
- 主窗口关闭后后台运行时会执行一次完整 GC，并调用 Windows `EmptyWorkingSet` 修剪工作集，降低任务管理器中可见的后台常驻占用。
- 回收动作会写入脱敏诊断日志 `memory / release-ui`，记录工作集、私有内存和托管堆，便于后续判断 UI 是否真正释放。
...
第一阶段已取消启动时自动 `LoadLogsAsync()`，第二阶段已将默认展示限制为最近 `500` 条。后续只需根据真实使用量决定是否在 UI 中增加“加载更多”。

### P2：日志读取改为流式限量

第二阶段已完成逐行读取和内存上限控制。后续如日志体量继续放大，可再优化为从文件尾部逆序读取，以减少解析旧记录的 CPU 时间。

### P2：后台状态采样加入诊断指标

第二阶段已加入脱敏进程指标。后续需要通过用户导出的诊断包确认这些指标在普通权限、管理员权限、安装版和便携版中都能正常生成。

## 7. 当前结论

优化前高内存更像架构性常驻开销，而不是单一泄漏。最核心的问题是：托盘模式创建并持有完整 WPF 主窗口、完整 ViewModel、UI 绑定集合、日志集合和 UI 级网络监控循环。

第一阶段已把“托盘后台 Host”和“主窗口 UI”拆开；第二阶段已把执行日志读取和诊断指标补齐；第三阶段已让“打开过主窗口后关闭到托盘”释放 UI。下一步应对新构建产物重新采样优化后工作集和私有内存。
