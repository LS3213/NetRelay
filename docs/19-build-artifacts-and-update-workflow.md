...
| 更新通道 | 当前固定为 `stable`，后台自动写入，维护者无需再选择或维护 `beta` |
| 平台架构 | `win-x64` |
| 最低可升级版本 | 允许直接在线升级的最低客户端版本 |
| 更新日志 | 清晰描述用户可感知变化和重要修复 |
| 强制更新 | 仅用于存在安全风险、协议不兼容或旧版无法继续使用的情况；普通功能更新不得滥用 |
...
- 若后端已启用 GitHub 自动同步，发布稳定版时会同步执行两类操作：一是更新 GitHub Release 中的 `win-x64.zip` 资源，二是更新 `gh-pages`（默认分支名可改）上的 `updates/stable/win-x64/latest.json` 与 `history.json` 元数据。
- GitHub 自动同步默认关闭；启用后若 GitHub 写入失败，后台“发布”或“撤回”会整体失败并回滚数据库状态，不会出现主站已发布但 GitHub 备用源未同步的半成功状态。
...
- 客户端启动时自动检查更新：无新版必须静默；有新版展示版本、发布时间和更新日志，普通更新可取消，强制更新不可取消。
- 关于页手动检查更新必须复用同一更新确认窗口，不得在用户确认前直接下载；关于页更新历史只展示仍处于已发布状态的版本，已撤回版本不可见。
- 管理后台“清理旧包”只删除超过保留数量的旧版 ZIP 文件，不撤回版本记录；客户端更新历史仍展示这些已发布版本，但直接下载已清理包会返回 404。当前只清理 `stable / win-x64`，默认保留最新 3 个可下载已发布包。
- 用户确认更新后，主程序负责下载并校验更新包，然后启动独立更新器。独立更新器必须显示与客户端浅色磨砂风格一致的进度窗口，覆盖等待主程序退出、清单校验、旧版备份、新版写入、启动自检和回滚阶段。
- 独立更新器完成后必须显示明确结果：成功时提示已更新并启动新版本；失败或自检不通过时提示已尝试回滚并应重新打开客户端或导出诊断日志。不得退回到无反馈的终端闪退体验。

### 4.2 诊断日志与反馈附件规范

- 客户端诊断日志写入 `%LOCALAPPDATA%\NetRelay\logs\diagnostic-YYYY-MM-DD.jsonl`，记录启动、更新检查、下载、校验、反馈提交等关键阶段。
- 诊断事件应包含组件、操作、结果、HTTP 状态、请求 ID、字节数、异常类型和 HResult 等定位信息。
- 禁止记录认证令牌、Cookie、CSRF、签名私钥、激活凭据、完整设备指纹、请求正文、服务端响应正文、完整用户目录和带查询参数的 URL。
- 建议反馈勾选附带日志时，压缩包必须同时包含规则执行日志、脱敏诊断日志和脱敏后的独立更新器日志；手动导出不得附带原始 `config.json`，只能输出不含凭据的运行摘要。
...
- 上传草稿后必须核对版本、固定通道 `stable`、固定架构 `win-x64`、大小和 SHA256，再执行发布。
...
6. 若启用了 GitHub 自动同步，同时确认 GitHub Release 资源和 `gh-pages/updates/stable/win-x64/latest.json`、`history.json` 已更新。
7. 使用旧版本客户端执行一次真实检查、下载、安装和启动验证。
...
| 更新器窗口停在失败提示 | 签名、哈希、包大小、自检或文件写入任一阶段失败，更新器已按日志记录并尝试回滚 | 查看 `%LOCALAPPDATA%\NetRelay\logs\updater-YYYY-MM-DD.log` 和反馈附件中的脱敏 updater 日志；确认服务端记录的包大小与 SHA256 和客户端下载文件一致 |
