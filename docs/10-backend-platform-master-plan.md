...
| 后续离线 | 首次激活成功后允许离线运行 |
| 设备指纹 | 优先验证并采用 `HardwareIds.NET`，不自行实现底层硬件枚举 |
...
HTTPS 是传输保护，不足以单独证明更新、封锁和公告内容可信。以下服务器响应必须使用离线保存的发布私钥签名：
...
首选候选为 `HardwareIds.NET`，原因是它提供成熟的硬件枚举能力，避免 NetRelay 自行实现 WMI、磁盘 IOCTL、显示器和硬件标识采集。

当前验证结论：

- `HardwareIds.NET` 使用 MIT 许可证。
- `1.1.9` 提供 .NET 8 兼容目标，适合当前客户端原型验证。
- 较新版本的目标框架与依赖需要在升级客户端框架前重新评估。
- 该项目采用量有限，因此在正式用于封锁前必须完成兼容性、稳定性、性能和隐私审计。

不得直接依赖某个库返回的单一最终 HWID。客户端需要通过可替换的 `IDeviceFingerprintProvider` 接口隔离第三方实现，避免未来无法替换组件。

### 7.2 禁止采集

即使第三方组件支持，NetRelay 也禁止采集或扫描：
...
```csharp
new HardwareIdsConfig
{
    ScanLocalNetworkDevices = false,
    ScanNeighborEndpoints = false
}
```
...
- 验证 `HardwareIds.NET 1.1.9` 在 .NET 8 WPF 中的兼容性。
- 验证关闭网络扫描后实际不会访问局域网邻居。
- 测试换网卡、换硬盘、虚拟机和重装系统后的指纹稳定性。
- 确定域名、HTTPS、GitHub 仓库和服务器文件目录。
...
- `HardwareIds.NET 1.1.9` 的具体字段稳定性、耗时和异常行为。
- 第三方指纹组件关闭网络扫描后的实际网络行为。
- 设备证据的匹配权重和合理误封阈值。
- 正式域名、GitHub 仓库地址和文件存储容量。
- 更新签名算法、密钥轮换和离线签名工作流的最终实现。
- 管理后台采用的具体前端框架；不得影响本文 API 与安全边界。
