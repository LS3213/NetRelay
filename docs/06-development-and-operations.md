...
- “使用帮助”只面向普通用户说明如何使用软件、概念含义、可见状态和常见处理方式。
- 不得在帮助中暴露后端地址、仓库地址、接口、签名、清单、证书、内部文件路径、发布同步细节、维护命令或其他开发/运维实现逻辑。
- 涉及更新来源时，客户端用户界面只使用“主更新源”和“备用源”等抽象名称，不直接展示备用源具体平台名称。
- 每次新增或调整用户可见功能、设置项、提示文案、更新流程、反馈流程或安全边界时，都要判断是否需要同步修改 `HelpDialog` 的用户帮助内容；若用户可能因新功能产生操作疑问，应同步更新。
