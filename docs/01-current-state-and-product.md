...
- 倒计时预警悬浮卡片与 Windows Toast 会按规则配置显示“延迟 N 分钟”；延迟和取消操作均绑定到当前通知实例并进行一次性令牌校验。
...
仍未实现或未完成真机确认的内容包括 NLM/NCSI 辅助信号、`ping` 诊断、可交互 Windows Toast 按钮的真机点击验收、安装包与代码签名。
...
3. **可交互式 Windows Toast 富通知（已实现，待真机点击验收）**：
   - 规则预警时按权限发送包含“延迟 N 分钟”、“取消本次”等快捷按钮的 Windows Toast 通知，允许用户无需打开主界面即可直接控制当前规则发生项。
4. **网卡防抖策略与冷却配置的可视化**：
