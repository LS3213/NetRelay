...
当前已经完成第四阶段的主要界面与自动化功能，并正在根据验收结果修复可靠性问题。当前代码已通过 Release 构建和仓库内回归测试；真实网卡切换、休眠恢复与不同 Windows 版本仍需真机验收。
...
仍未实现或未完成真机确认的内容包括 NLM/NCSI 辅助信号、`ping` 诊断、可交互 Windows Toast 按钮、应用诊断日志、日志自动滚动清理、安装包与代码签名，以及按钮 UI 视觉样式的最终微调美化（目前版本的按钮样式不佳，标记为待后续重新设计和修改）。
...
2. **日志自动滚动与容量清理（Log Rotation）**：
   - 限制 `%LOCALAPPDATA%\NetRelay\logs\` 目录下日志文件的最大存储天数或总容量，到期自动清理过期日志，避免磁盘占用膨胀。
...
5. **基于 WiFi SSID 等网络环境的自适应规则**：
   - 支持根据当前连接的 WiFi 名称（SSID）或网关 MAC，自动挂起或激活特定规则（例如：仅在连接学校宿舍网络时激活 23:00 断网规则）。
6. **引导式安装包（Installer）**：
...
7. **单进程内部异常隔离与优雅退出**：
   ...
   - **异常安全隔离**：对 Windows 原生 COM 接口调用与网络状态获取增加更周密的异常捕获隔离，避免偶发系统 API 错误引发主进程意外崩溃。
