...
> 状态：B0 设计基线与原型结论。本文定义不可绕过的信任和隐私边界；正式算法库、密钥设施和设备指纹供应器仍待 B0 验收。
...
| 项目 | 当前结论 |
| --- | --- |
| DeviceId API、维护度与采集范围 | 已确认更适合作为首选；运行和抓包验证进行中 |
| DeviceId + NetRelay 网卡组合原型 | 6 类核心硬件证据与 3 类网卡辅助证据运行通过；连续运行数量一致，未输出原始值 |
| 原始标识不上传设计 | 已确定；正式实现待 B2 |
| 签名与重放原型 | 原型已建立，待运行验证 |
| 更新一致性与回滚原型 | 原型已建立，待运行验证 |
