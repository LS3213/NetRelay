...
> **历史文档提示（2026-07-30）**：本文记录旧 NetRelay Server 的安全基线，不再定义当前 Omnexa 接入语义。当前以 [NetRelay 接入 Omnexa](21-omnexa-client-integration.md) 为准：后台只管理设备；历史协议字段 `installationId` 现在表示按程序目录持久化的内部运行槽位，同目录升级复用、不同目录隔离，槽位不展示也不作为封禁目标。
...
- B7 反馈历史查询只允许按客户端本地匿名设备指纹返回该设备提交过的反馈标题、正文、状态和时间；不得返回附件下载链接、联系方式以外的其他设备数据，管理端附件下载仍必须走已认证管理 API。
