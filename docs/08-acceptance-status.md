...
截至 2026-06-14，NetRelay 客户端及后端平台 B0-B4 阶段（包括双源更新、反馈、日志上传与公告系统）的代码实现与验收已全部完成，自动化质量门槛已全部通过。Windows Toast 真实点击、物理网卡切换、休眠、重新登录和跨 Windows 版本行为尚未手动实机验收，不得视为全部通过。
...
| 仓库回归测试 | 41 项通过（含反馈日志打包及 SafeMarkdownParser 安全过滤测试） |
| 服务端集成测试 | 24 项通过（含设备激活、更新分发、反馈列表/下载及公告签名校验） |
| Markdown 相对链接 | 通过 |
| Git 差异空白检查 | 通过 |
| 探测配置校验与自动化暂停 | 通过 |
