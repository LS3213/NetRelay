...
| --- | --- | --- | --- | --- |
| B0 | 规范、原型与风险验证 | 未开始 | 无 | 总体规范已建立，原型与专项设计尚未执行 |
...
| 负责人 | 未填写 |
| 相关 Commit | 未填写 |
| 阻塞项 | 正式域名、GitHub 仓库、管理后台技术栈和签名工作流待确认 |
...
- [ ] 定义 API 版本、统一响应、错误码、分页、时间和 ID 格式。
- [ ] 定义 OpenAPI 初稿和客户端/服务器共享 DTO 边界。
- [ ] 定义 MySQL 表、字段、索引、外键、保留和删除策略。
- [ ] 定义签名算法、序列化规范、密钥生成、轮换、吊销和离线签名流程。
- [ ] 定义隐私政策、用户协议、同意版本和重新同意规则。
- [ ] 定义威胁模型、信任边界、受限模式边界和管理员审计要求。
- [ ] 定义 Ubuntu 测试与生产环境、域名、目录、备份和恢复方案。

#### B0.2 HardwareIds.NET 原型

- [ ] 创建隔离原型，不接入正式客户端业务流程。
- [ ] 通过 `IDeviceFingerprintProvider` 隔离第三方库。
- [ ] 明确关闭局域网、邻居端点和附近 Wi-Fi 扫描。
- [ ] 记录 HardwareIds.NET 版本、许可证、依赖和升级风险。
...
- [ ] 完成规范化 JSON 或等价确定性序列化原型。
- [ ] 完成签名、验证、过期、nonce 和重放拒绝原型。
- [ ] 验证主源和 GitHub 可使用完全相同的清单、签名和更新包。
- [ ] 验证独立更新器替换与回滚方案的可行性，不接入正式更新按钮。

#### B0.4 管理后台与部署决策

- [ ] 确认管理后台具体技术栈和构建方式。
- [ ] 确认 Ubuntu、Nginx、MySQL、文件目录与 Docker Compose 拓扑。
- [ ] 确认服务器更新包和私有反馈附件的隔离方式。
- [ ] 确认正式域名、GitHub 仓库和证书管理方式。
...
- 设备指纹与签名原型报告。
- 风险结论：采用、调整或放弃 HardwareIds.NET，并说明证据。
...
| 指纹稳定性 | 形成明确测试矩阵、变化结果和误封风险结论 | 未填写 | 未开始 |
| 原始标识保护 | 代码、请求和普通日志均不出现原始硬件序列号 | 未填写 | 未开始 |
| 签名原型 | 正常签名通过，篡改、过期、重放和错误适用范围均拒绝 | 未填写 | 未开始 |
| 更新原型 | 主源与 GitHub 相同产物可验证，损坏包被拒绝并可回滚 | 未填写 | 未开始 |
| 文档评审 | API、数据库、安全、隐私和部署专项文档无未解释冲突 | 未填写 | 未开始 |
