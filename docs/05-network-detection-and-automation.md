...
探测策略会在加载和保存时进行严格校验：端点至少一个且必须为 HTTP/HTTPS 绝对地址，超时限制为 0.5 到 30 秒，探测轮数限制为 1 到 10，失败轮数与单轮成功端点数必须处于有效范围。无效策略不会被解释为在线，并会暂停依赖探测的自动规则执行；手动操作与已排队的安全恢复仍可执行。
...
- 恢复来源不受原禁用规则冷却时间和附加条件限制，也不会覆盖普通规则的最后执行时间。
- 如果用户在此期间手动启用了网卡，自动恢复计时到点后记录幂等成功，不重复操作。
