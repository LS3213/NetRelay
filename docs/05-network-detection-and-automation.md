...
- **重启去重机制**：为了防止由于关闭并重启应用，导致已在今日执行完毕的定时规则（Daily/Weekly）在启动时因内存状态丢失而再次误触发，调度器在启动（`Start`）时会异步扫描当天的本地结构化日志文件（`execution-YYYY-MM-DD.jsonl`），将已运行记录载入内存。对于今天已经触发过的规则，当日重启将不会再次运行。
