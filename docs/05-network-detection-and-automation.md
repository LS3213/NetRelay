...
> 当前已实现网卡状态检查、多协议探测（支持 HTTP/HTTPS/PING/DNS 协议）、连续失败判定、网络离线边沿触发、备用网络保护、切换后回滚以及 Windows 原生 NLM/NCSI COM 状态与 UI 联动。核心策略已有自动化回归测试；复杂校园网、代理和多路由环境仍需真机验证。
...
- **DNS UDP 协议**（`dns://<待解析域名>`）：通过本地单播 IP 绑定 UDP 端口 53 构造标准的 DNS 主机 A 记录或 AAAA 记录查询（支持 IPv4/IPv6），发送到网卡配置的 DNS 服务器（若为空则使用公共 DNS 服务器 `223.5.5.5` 或 `2400:3200::1`）。
